...
class LexedItem:
    ...
    def filter(self,keep=None,remove=None):
        ...
        if not keep:
            a = Lexer()
            a.lex('1')
            keep = ['FLOAT', 'INT', 'IDENTIFIER', 'STR', 'COMMENT', 'MULT', 'DIV', 'PLUS','MINUS', 'LPAREN', 'RPAREN', 'SPACE', 'NEWLINE',
                    'SEMICOLON', 'COLON', 'RCURLY', 'LCURLY', 'LSQUARE', 'RSQUARE', 'COMMA', 'ASSIGN', 'LTE', 'GTE', 'LT', 'GT', 'EQ',
                    'FULLSTOP', 'EXCLAIM']
...
class Parser:
    ...
    def parse(self,tokens):
        disallowed = ('SPACE','COMMENT','NEWLINE','TAB')
        self.tokens = []
        self.current_token = None
        self.i = -1
    ...
    def factor(self):
            ...
            self.raise_error()
print(Lexer().lex('1.2 * 5').reconstruct_file('a.txt'))
